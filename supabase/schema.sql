...
create table if not exists public.admin_logins (
  ...
  name            text        not null,                    -- who signed in, entered at login
  logged_in_at    timestamptz not null default now()
);

create index if not exists admin_logins_logged_in_at_idx on public.admin_logins (logged_in_at desc);
...
drop policy if exists admin_logins_insert       on public.admin_logins;
drop policy if exists admin_logins_select       on public.admin_logins;
...
-- Admin login history is append-only — insert and read, but no update/delete
-- policy is defined, so the audit trail can't be edited or erased from the app.
